...
- **Play-progress + quest state must persist:** checking a step off, or giving up / resuming a quest, writes back to the stored quest (not just the working draft), so it survives leaving + re-opening and updates the map (grey bar + faded flag for given-up). Give-up is reversible — the builder button toggles to **↩ resume quest**. (Prototype does this via `syncQuestToWorld()`; the live game must do the same.)
